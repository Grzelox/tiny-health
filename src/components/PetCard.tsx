...
export default function PetCard({ pet, onDelete }: PetCardProps) {
  ...
  return (
    <div className={`relative block h-64 ${isDead ? 'bg-gray-200' : 'bg-white'} rounded-lg shadow-sm hover:shadow-md transition-shadow`}>
      <div className="p-6 h-full flex flex-col relative z-10">
        ...
        <div className="flex-grow">
          <h3 className={`text-xl font-semibold ${isDead ? 'text-black' : 'text-primary-800'} mb-2`}>{pet.name}</h3>
          <p className={`text-secondary-600 ${isDead ? 'text-gray-600' : ''}`}>{pet.breed}</p>
        </div>
        <div className="mt-4">
          <Link href={`/pet/${pet.id}`}>
            <span className={`text-sm ${isDead ? 'text-black' : 'text-primary-600'} hover:${isDead ? 'text-gray-700' : 'text-primary-700'}`}>
              Szczegóły →
            </span>
          </Link>
        </div>
